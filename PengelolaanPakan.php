...
include "INCLUDES/confiig.php";
	...
	if(isset($_POST["Batal"])){
    	...
    	mysqli_query($connection,"delete from pengelolaanpakandtemp where KodePengelolaan = '$kodejadi'");
    ...
    if(isset($_POST['hapus']))
    {
    	...
    	header("location:pengelolaanPakanhapus.php?hapus=$nomor");
    }
	...
	if(isset($_POST['SimpanTemp'])){
    	...
    	if ($Total < 0)
    	{
    		die('Total tidak boleh kurang dari 0');
    	}
    	if (!empty($Total))
    	{
    	...
    	{
    		$existed = mysqli_query($connection,"select * from persediaan p join pengelolaanpakandtemp pt on p.kodePersediaan = pt.KodePersediaan where pt.kodePersediaan = '$persediaan'");
    		if(mysqli_num_rows($existed)>0){
    			...
    			$subtotalstdpro2 = $stdpro * $totalsekarang;
    			mysqli_query($connection,"update pengelolaanpakandtemp set TotalPenggunaan='$totalsekarang', SubTotalProtein='$subtotalstdpro2' WHERE Nomor ='$nomor'");
    		}
    		else{
    			$subtotalstdpro = $stdpro * $Total;
...
<button type="button" class="btn btn-info" data-toggle="modal" data-target="#myModal">Tambah bahan</button>
      ...
      <div class="modal-body">
        <form method="POST">
        	<input type="hidden" readonly class="form-control" id="kodePemasukanBarang" name="inputPemasukanBarang"
			 	...
			 	<div class="form-group row">
				    <label for="persediaan" class="col-sm-4 col-form-label d-flex">Persediaan</label>
					    <div class="col-sm-8">
						      ...
						      <option value="<?php echo $row["KodePersediaan"]?>">
						        <?php echo $row["NamaPersediaan"]?> | <?php echo $row["StandarProtein"]?>
						      </option>
						    	<?php } ?>
						   	</select>
					    </div>
			   	...
			   	<div class="form-group row">
				    <label for="TotalPemasukan" class="col-sm-4 col-form-label d-flex">Total Penggunaan</label>
					    <div class="col-sm-8">
	 	...
	 	<table class="table table-striped">
			...
			<?php
	    		...
	    		$totalpakan = $row['TotalPenggunaan'] + $totalpakan;
					...
					<td style="text-align: center;"><?php echo $persediaan;?></td>
					<td style="text-align: center;"><?php echo number_format($stdpro);?></td>
					<td style="text-align: center;"><?php echo number_format($row['TotalPenggunaan']);?></td>
					<td style="text-align: center;"><?php echo number_format($row['SubTotalProtein']);?></td>
					<td>
